...
//        logger.log(.debug, "🛫 API Request\nHttp method: \(request.method)\nPath: \(baseURL.absoluteString.appending(request.path!))\nHeaders: \(request.headerFields.debugDescription) \nRequest id: \(UUID().uuidString)")
//        do {
//            let (response, responseBody) = try await next(request, body, baseURL)
//            
//            let responseBodyText: String = "Not implemented"
////            let copy = responseBody
////            if let copy {
////                // Read body once and decode it
////                let bodyData = try await Data(collecting: copy, upTo: 3)
////                        responseBodyText = String(decoding: bodyData, as: UTF8.self)
////            } else {
////                responseBodyText = "Empty"
////            }
//            
//            if 200...299 ~= response.status.code {
//                logger.log(.debug, "✅ API Response Success\nStatus: \(response.status) \nHttp body:\n\(responseBodyText)")
//            } else {
//                logger.log(.debug, "‼️ API Response Error\nStatus: \(response.status)\nHttp body:\n\(responseBodyText)")
//            }
//            return (response, responseBody)
//        } catch {
//            logger.log(.debug, "‼️ API Response \nUnexpected Error \n(\(error.localizedDescription)")
//            throw error
//        }
//    }
//}
...
package actor LoggingMiddleware {
    ...
    package init(logger: Logger = defaultLogger, bodyLoggingConfiguration: BodyLoggingPolicy = .upTo(maxBytes: 10_0000_00000)) {
        self.logger = logger
